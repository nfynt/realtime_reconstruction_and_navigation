...
#define OPENCV_API __declspec(dllexport) __stdcall
...
String _windowName = "InteropSample";
VideoCapture _capture;
int _scale = 1;

extern "C" int OPENCV_API Init(int& outCameraWidth, int& outCameraHeight)
{
...
extern "C" void OPENCV_API  Close()
{
	_capture.release();
}

extern "C" void OPENCV_API SetScale(int scale)
{
	_scale = scale;
}

extern "C" void OPENCV_API Detect(Circle* outFaces, int maxOutFacesCount, int& outDetectedFacesCount)
{
	Mat frame;
	_capture >> frame;
	if (frame.empty())
